/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_list_find.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: diogpere <diogpere@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/13 11:23:39 by diogpere          #+#    #+#             */
/*   Updated: 2023/02/14 18:02:38 by diogpere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_list.h"

t_list	*ft_list_find(t_list *begin_list, void *data_ref, int (*cmp)())
{
	t_list	*guide;

	guide = begin_list;
	while (guide != 0)
	{
		if ((*cmp)(guide->data, data_ref) == 0)
			return (guide);
		guide = guide->next;
	}
	return (0);
}

/////////////////////////////////////////////////

// t_list	*ft_create_elem(void *data)
// {
// 	t_list	*ret;

// 	ret = malloc(sizeof(t_list));
// 	if (ret != NULL)
// 	{
// 		ret->data = data;
// 		ret->next = NULL;
// 	}
// 	return (ret);
// }

// void	ft_list_push_front(t_list **begin_list, void *data)
// {
// 	t_list	*temp;

// 	if (begin_list == 0)
// 		return ;
// 	temp = ft_create_elem(data);
// 	if (temp == 0)
// 		return ;
// 	temp->next = *begin_list;
// 	*begin_list = temp;
// }

// t_list	*ft_list_push_strs(int size, char **strs)
// {
// 	t_list	*ret;
// 	int		i;

// 	i = 0;
// 	ret = 0;
// 	while (i < size)
// 	{
// 		ft_list_push_front(&ret, strs[i]);
// 		++i;
// 	}
// 	return (ret);
// }

// void	print_list(t_list *list)
// {
// 	while (list != NULL)
// 	{
// 		printf("\"%s\" -> ", (char *)list->data);
// 		list = list->next;
// 	}
// 	printf("NULL\n");
// }

// int	main(void)
// {
// 	char *test0[] = {"blablabla", "abacaxi", "banana", "árvore", "abacaxi"};
// 	char *test1[] = {"vaca", "cavalo", "galinha", "cavalo", "porco", "ovelha"};

// 	t_list *list0 = ft_list_push_strs(5, test0);
// 	t_list *list1 = ft_list_push_strs(6, test1);
// 	;

// 	printf("list: ");
// 	print_list(list0);
// 	printf("=> looking for \"abacaxi\", found %s\n",
// 			(char *)(ft_list_find(list0, "abacaxi", (int (*)())strcmp))->data);
// 	printf("=> looking for \"a\", found %p\n", ft_list_find(list0, "a",
// 				(int (*)())strcmp));

// 	printf("\nlist: ");
// 	print_list(list1);
// 	printf("=> looking for \"cavalo\", found %s\n", (char *)(ft_list_find(list1,
// 					"cavalo", (int (*)())strcmp))->data);
// 	printf("=> looking for \"c\", found %p\n", ft_list_find(list1, "c",
// 				(int (*)())strcmp));
// }